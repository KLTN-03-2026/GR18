...
async function handleGoogleLogin(response) {
    ...
    try {
        const res = await axios.post(`${window.API_BASE}/auth/google`, {
            token: idToken
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await axios.post(`${window.API_BASE}/auth/login`, payload);
    ...
    if (sendOtpBtn) {
        sendOtpBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await axios.post(`${window.API_BASE}/auth/forgot-password`, { email });
    ...
    if (forgotPasswordForm) {
        forgotPasswordForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await axios.post(`${window.API_BASE}/auth/reset-password`, {
                    email,
                    otpCode,
                    newPassword
                });
